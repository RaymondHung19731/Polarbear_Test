...
public class HexByteKit {
	private static final String TAG = HexByteKit.class.getSimpleName();
    ...
    public static String Byte2Hex(byte[] b, int offset, int len, String sep, int endian)
    {
        ...
        else // little-endian
        {
        	...
        	stmp = (Integer.toHexString(b[bdy] & 0xFF));
            if(stmp.length() == 1) {
                hexStr.append("0"+stmp);
            } else {
                hexStr.append(stmp);
