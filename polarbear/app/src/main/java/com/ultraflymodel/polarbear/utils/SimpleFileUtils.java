...
import com.ultraflymodel.polarbear.preview.IconPreview;
import com.ultraflymodel.polarbear.preview.MimeTypes;
...
public class SimpleFileUtils {
    private static final String TAG = SimpleFileUtils.class.getSimpleName();
...
/*
    ...
    private static void search_file(String dir, String fileName, ArrayList<String> n) {
        HILog.d(TAG, "search_file: dir = " + dir + ", fileName = " + fileName);
        File rootDir = new File(dir);
        String[] list = rootDir.list();
        boolean root = Settings.rootAccess();
        HILog.d(TAG, "search_file: list.length = " + list.length);
        ...
        if (list != null && rootDir.canRead()) {
            ...
            HILog.d(TAG, "Can not read dir list for unknown reason!");
            if (root)
...
*/
    ...
    public static void createShortcut(Activity main, String path) {
        ...
        try {
            ...
            if (file.isFile()) {
                ...
                shortcutIntent = new Intent(main, PolarbearMainActivity.class);
            ...
            if (file.isFile()) {
                ...
                if (bd != null) {
                    ...
                    intent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
                            Intent.ShortcutIconResource.fromContext(main, R.mipmap.type_unknown));
                }
            } else {
                intent.putExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE,
    ...
    public static BigInteger CalculatedMBSize(long ls) {
        BigInteger size = BigInteger.valueOf(ls);
        BigInteger MBsize = BigInteger.ZERO;

        if (size.divide(MB_BI).compareTo(BigInteger.ZERO) > 0)
            MBsize = size.divide(MB_BI);

        return MBsize;
    }

    public static BigInteger CalculatedKBSize(long ls) {
        BigInteger size = BigInteger.valueOf(ls);
        BigInteger KBsize = BigInteger.ZERO;

        if (size.divide(KB_BI).compareTo(BigInteger.ZERO) > 0)
            KBsize = size.divide(KB_BI);
