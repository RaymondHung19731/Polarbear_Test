package com.ultraflymodel.polarbear.preview;
...
import com.ultraflymodel.polarbear.settings.Settings;
...
public class IconPreview {
    ...
    private static boolean isvalidMimeType(File file) {
        ...
        boolean isMp3 = file.getName().endsWith(".mp3");
        return isImage || isVideo || isApk || isMp3;
    }
    ...
    private static void loadFromRes(final File file, final ImageView icon) {
        ...
        if (file != null && file.isDirectory()) {
            ...
            if(!StringUtil.isStrNullOrEmpty(file.getName())) fileExt = SimpleFileUtils.getExtension(file.getName());
            if(!StringUtil.isStrNullOrEmpty(fileExt)) mimeIcon = mMimeTypeIconCache.get(fileExt);
        ...
        if (mimeIcon != null) {
            ...
            // default icon
            icon.setImageResource(R.mipmap.type_unknown);
        }
    }
